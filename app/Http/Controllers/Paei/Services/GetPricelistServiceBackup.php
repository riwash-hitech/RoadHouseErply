...
class GetPricelistService
{
        ...
        if (@$product["pricelistRules"]) {
            foreach (@$product['pricelistRules'] as $rule) {
                ...
                if ($oldPriceRule) {
                    ...
                    //if price is same then not to update pending to 1
                    $details["isDeleted"] = 0;
                    if ($oldPriceRule->isDeleted == 1 || $oldPriceRule->price != @$rule["price"]) {

                        //if price changed then update price pending
                        $isNew = 1;
                        // $this->updateMatrixProductPricePending($rule["id"], 1);
                        $details["pendingProcess"] = 1;
                        // $this->updateMatrixProductPricePending($rule["id"], 1);
                    } else {
                        if ($oldPriceRule->isDeleted == 1) {
                            //if price rule deleted previously the update price pending 1
                            $details["pendingProcess"] = 1;
                        }
                        // else{
                        //     //if price not changed and is not deleted 
                        //     $vp = VariationProduct::where("productID", $rule["id"])->first();
                        //     $this->updateMatrixProductPricePending($rule["id"], $vp->pricePending, 1);
                        // }
                    ...
                    // this section means new price list rules updated or exist  
    ...
    private function updateMatrixProductPricePending($productID, $pendingFlag)
    {
        //first getting variation product 

        $vp = VariationProduct::where("productID", $productID)->first();
