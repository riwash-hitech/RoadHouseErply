...
class CustomerService{
    ...
    public function syncCustomerToErply(){
        ...
        $customers = ShopifyCustomer::where("erplyPending", 1)
                        ->limit(50) 
                        ->get();
                        
        if($customers->isEmpty()){
            info("All Customer  Synced to Erply.");
            ShopifyCustomer::where("erplyPending", 2)->update(["erplyPending" => 1]);
        ...
        if($res['status']['errorCode'] == 0 && !empty($res['requests'])){
            foreach($customers as $key => $c){
                if($res['requests'][$key]['status']['errorCode'] == 0){
                    ...
                    $c->erplyError = json_encode($res['requests'][$key], true); 
                    $c->save();
                }
            }
