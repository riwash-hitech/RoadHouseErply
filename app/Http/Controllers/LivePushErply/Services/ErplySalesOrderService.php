...
class ErplySalesOrderService{
    ...
    public function pushSalesOrders($req){
        ...
        foreach($salesOrders as $so){
            ...
            $disAmt = 0;
            $disAmtIncGst = 0;
            if((double)$so->coupon_amount > 0){
                $disWithouGst = (double)$so->coupon_amount;// /1.1;
                ...
                $disAmt = (double)$disWithouGst / $count;
            } 
            ...
            if($so->total_shipping > 0){
                ...
                $reqArray["price".$count+1] = (double)$so->total_shipping / 1.1;//- (double)$so->coupon_amount;
            }
       ...
       if($result->isEmpty()){
        ...
        foreach($result as $so){
            ...
            if($so->refund_shipping_amount > 0){
                    ...
                    $reqArray["amount".$count+1] = -1;
                    $reqArray["discount".$count+1] = 0;
                    $reqArray["price".$count+1] =  (double)$so->refund_shipping_amount / 1.1;//- (double)$so->coupon_amount;
                    $isPayload = 1;
            }
    ...
    public function pushRefundReturnV2 ($req) 
    {
        ...
        if (request()->query('debug') == 1) dd($result);
        ...
        foreach ($result as $so) {
            ...
            if ($so->refund_product_code != '') {
                ...
                foreach ($returnItems as $key => $code) {
                    ...
                    if (empty($req_array_2)) continue; // skip
                    ...
                    if (empty($product)) continue; // Skip

                    // now set product stock pending
                    if (@$product->parentProductID > 0) MatrixProduct::where("productID", $product->parentProductID)->update(["shopifySohPending" => 1]);
            ...
            // dd($bundleArray);
            if (count($req_refunds) < 1) continue; // Skip

            if (request()->query('debug') == 2) dd('Bundle Array : ', $bundle_array);
    ...
    public function createRequest($so, $locationId = null) 
    {
        ...
        if ($locationId == null) {
            ...
            if ($location) $location_id = $location->warehouseID;
        }
        if ($location_id != null) {
            return [
                "requestName" => "saveSalesDocument",
